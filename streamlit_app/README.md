...
3. **Find the embedding notebook's workspace path.** In the Databricks workspace file browser, locate `notebooks/05_generate_embeddings_for_new_frag` and copy its full path (e.g. `/Workspace/Repos/<you>/sniffers/notebooks/05_generate_embeddings_for_new_frag`). Needed for the "Add a fragrance" tab, which triggers this notebook as a one-time job run instead of running the embedding model locally.
...
- The "Add a fragrance" tab needs `notebook_path` set in `secrets.toml`. If the job fails with a cluster-related error, your workspace may not have serverless job compute enabled — set `job_cluster_id` in `secrets.toml` to an existing all-purpose cluster's ID instead.
